...
class SnsHomeViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource{
    @IBOutlet var SnsTableView: UITableView!
    var snss = [Sns]()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! SnsTableViewCell
        let sns: Sns
        sns = snss[indexPath.row]
        
        cell.snsText.text = sns.text
        
        if sns.picture != nil {
            let url = URL(string: sns.picture!)!
            cell.snsImage.af_setImage(withURL: url)
        }
        return cell
    }
